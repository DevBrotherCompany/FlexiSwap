...
async function mintTokens(
	...
	return new Array(10).fill(0).map((_, i) => [token.address, i]);
}
...
async function approve(
	...
	await Promise.all(tokenIds.map((tokenId) => token.approve(spender, tokenId)));
}
...
describe("FlexiSwap contract", function () {
	...
	describe("mint", function () {
		it("Should mint tokens", async function () {
			const { owner, addr1, addr2, hardhatTestToken } = await loadFixture(
				deployTestTokenFixture
			);

			const tokens = await mintTokens(hardhatTestToken, owner.address, 10);
